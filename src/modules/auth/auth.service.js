...
//   const otp            = generateOTP();          // ✅ بدل Math.random
//   const hashedOTP      = hashToken(otp);         // ✅ هنحفظ الـ hash مش الـ OTP

//   const user = await User.create({
//     name,
//     email,
//     password       : hashedPassword,
//     confirmEmailOTP    : hashedOTP,              // ✅ hash في الـ DB
//     confirmEmailExpires: Date.now() + 10 * 60 * 1000,
//     isConfirmed    : false,
//   });

//   console.log(`OTP for ${email}: ${otp}`);      // الـ OTP الأصلي في الـ console

//   return {
//     userId: user._id,
//     email : user.email,
//     name  : user.name,
//     otp,                                         // ✅ بنرجع الأصلي للـ Flutter
//   };
// };
...
//   if (user.confirmEmailOTP !== hashToken(otp))  // ✅ بنقارن الـ hash بالـ hash
...
//   const otp       = generateOTP();               // ✅ بدل Math.random
//   const hashedOTP = hashToken(otp);              // ✅ hash في الـ DB
...
//   if (user.resetPasswordOTP !== hashToken(otp)) // ✅ بنقارن الـ hash بالـ hash
//     throw new Error('Invalid OTP');
...
export const loginService = async ({ email, password }) => {
  ...
  return {
    ...
    user: {
      ...
      name        : user.name,
      email       : user.email,
      profileImage: user.profileImage, // ✅ رجّعها في الـ login response
    },
  };
};
