...
//       if (!content) continue; // جرب الموديل الجاي

//       console.log(`✅ Used model: ${model}`);
//       return content;
//     } catch (error) {
//       const code = error.response?.data?.error?.code;
//       console.warn(`⚠️ Model ${model} failed (${code}), trying next...`);
//       if (code === 429) {
//         await new Promise((res) => setTimeout(res, 2000)); // استنى 2 ثانية
//       }
//       continue;
//     }
//   }
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    for (const model of FREE_MODELS) {
      try {
        ...
        const content = response.data?.choices?.[0]?.message?.content;
        if (!content) {
          console.warn(`⚠️ Model ${model} returned empty content, skipping...`);
          continue;
        }

        console.log(`✅ Used model: ${model} (attempt ${attempt + 1})`);
        return content;
        ...
        console.warn(`⚠️ Model ${model} failed (code: ${code}, status: ${status}), trying next...`);
    ...
    if (attempt < retries) {
      console.warn(`🔄 All models failed on attempt ${attempt + 1}, retrying in 3s...`);
